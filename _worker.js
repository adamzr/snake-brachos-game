// The HTML content for the game will be placed here.
// Due to its size, it's embedded as a template literal.
const HTML_CONTENT = `
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .hebrew-text {
            font-family: 'Arial', 'Helvetica Neue', Helvetica, sans-serif; /* Common fonts with Hebrew support */
            direction: rtl;
        ...
        #gameCanvas {
            ...
            border-radius: 1rem; /* Rounded corners for the game area */
        ...
        .pixelated-font { /* For a retro game feel if desired, otherwise use Inter */
        ...
        .snake-head-color { fill: #22c55e; } /* green-500 */
        .snake-body-color-1 { fill: #34d399; } /* emerald-400 */
        .snake-body-color-2 { fill: #6ee7b7; } /* emerald-300 */

        /* Ensure canvas is responsive and maintains aspect ratio if needed, or fills container */
        #gameContainer {
            width: 100%;
            max-width: 500px; /* Max width for the game area on larger screens */
...
<body class="bg-sky-100 flex flex-col items-center justify-center min-h-screen p-4 overflow-hidden">
    ...
    <div id="splashScreen" class="absolute inset-0 flex flex-col items-center justify-center z-50 splash-screen">
        <h1 class="text-5xl font-bold text-sky-700 mb-4">Kosher Snake Adventure</h1>
        <p class="text-xl text-sky-600 mb-8">!נָחָשׁ כָּשֵׁר לְמַהְדְּרִין</p>
        <button id="playButton" class="btn-primary text-2xl px-8 py-4 rounded-lg shadow-lg hover:shadow-xl transition-shadow">
            Play Game
        </button>
        <p class="mt-8 text-sm text-gray-500">Use Arrow Keys or Swipe to Control</p>
    </div>

    <div id="gameOverScreen" class="hidden absolute inset-0 flex flex-col items-center justify-center z-40 game-over-screen">
        ...
        <button id="playAgainButton" class="btn-primary text-xl px-6 py-3 rounded-lg shadow-md hover:shadow-lg transition-shadow">
            Play Again
        </button>
    </div>
    ...
    <div id="gameArea" class="hidden w-full max-w-2xl flex flex-col items-center">
        ...
        <div id="touchControls" class="mt-4 grid grid-cols-3 gap-2 w-48 md:hidden">
            <div></div> <button data-direction="up" class="bg-sky-500 text-white p-3 rounded-lg shadow active:bg-sky-600 flex justify-center items-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><path fill-rule="evenodd" d="M11.47 2.47a.75.75 0 011.06 0l7.5 7.5a.75.75 0 11-1.06 1.06l-6.22-6.22V21a.75.75 0 01-1.5 0V4.81L4.03 11.03a.75.75 0 01-1.06-1.06l7.5-7.5z" clip-rule="evenodd" /></svg>
            </button>
            <div></div> <button data-direction="left" class="bg-sky-500 text-white p-3 rounded-lg shadow active:bg-sky-600 flex justify-center items-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><path fill-rule="evenodd" d="M2.47 11.47a.75.75 0 010-1.06l7.5-7.5a.75.75 0 111.06 1.06L4.81 9.25H21a.75.75 0 010 1.5H4.81l6.22 6.22a.75.75 0 11-1.06 1.06l-7.5-7.5z" clip-rule="evenodd" /></svg>
            </button>
            <button data-direction="down" class="bg-sky-500 text-white p-3 rounded-lg shadow active:bg-sky-600 flex justify-center items-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><path fill-rule="evenodd" d="M12.53 21.53a.75.75 0 01-1.06 0l-7.5-7.5a.75.75 0 011.06-1.06l6.22 6.22V3a.75.75 0 011.5 0v16.19l6.22-6.22a.75.75 0 111.06 1.06l-7.5 7.5z" clip-rule="evenodd" /></svg>
            </button>
            <button data-direction="right" class="bg-sky-500 text-white p-3 rounded-lg shadow active:bg-sky-600 flex justify-center items-center">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-6 h-6"><path fill-rule="evenodd" d="M21.53 12.53a.75.75 0 010-1.06l-7.5-7.5a.75.75 0 01-1.06 1.06L19.19 9.25H3a.75.75 0 010-1.5h16.19l-6.22-6.22a.75.75 0 011.06-1.06l7.5 7.5z" clip-rule="evenodd" /></svg>
            </button>
        </div>
    </div>
    ...
    <script>
        ...
        const POINTS_FOR_EXTRA_LIFE = 1000; // Every 1000 points, gain a life

        const LEVELS_DATA = [
            { name: "ברכת הָעֵץ", brachaText: "בָּרוּךְ אַתָּה ה׳ אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם בּוֹרֵא פְּרִי הָעֵץ", brachaType: "haetz", correctFoods: ["🍎", "🍊", "🍇", "🍐", "🍑", "🍒", "🍍", "🥭", "🥝"], wrongBrachaCategories: ["haadama", "mezonot", "shehakol"], targetLength: 8, speed: 220, foodDropInterval: 3000, maxFoodItems: 4 },
            { name: "ברכת הָאֲדָמָה", brachaText: "בָּרוּךְ אַתָּה ה׳ אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם בּוֹרֵא פְּרִי הָאֲדָמָה", brachaType: "haadama", correctFoods: ["🥕", "🍅", "🥦", "🌽", "🥒", "🥔", "🌶️", "🥬", "🍓", "🍉", "🧅", "🧄"], wrongBrachaCategories: ["haetz", "mezonot", "shehakol"], targetLength: 12, speed: 190, foodDropInterval: 2800, maxFoodItems: 5 },
            { name: "ברכת מְזוֹנוֹת", brachaText: "בָּרוּךְ אַתָּה ה׳ אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם בּוֹרֵא מִינֵי מְזוֹנוֹת", brachaType: "mezonot", correctFoods: ["🥨", "🥐", "🍪", "🎂", "🍰", "🧇", "🍩", "🍕", "🍝"], wrongBrachaCategories: ["haetz", "haadama", "shehakol"], targetLength: 16, speed: 160, foodDropInterval: 2500, maxFoodItems: 6 },
            { name: "ברכת שֶׁהַכֹּל", brachaText: "בָּרוּךְ אַתָּה ה׳ אֱלֹהֵינוּ מֶלֶךְ הָעוֹלָם שֶׁהַכֹּל נִהְיָה בִּדְבָרוֹ", brachaType: "shehakol", correctFoods: ["🥛", "🥚", "🧀", "🍄", "🐟", "🍬", "🍫", "🍿", "🥤", "🍭", "🍯", "🥩", "🍗"], wrongBrachaCategories: ["haetz", "haadama", "mezonot"], targetLength: 20, speed: 130, foodDropInterval: 2200, maxFoodItems: 7 },
            { name: "ברכה אחרונה: עַל הַמִּחְיָה", brachaText: "עַל הַמִּחְיָה וְעַל הַכַּלְכָּלָה... (נוסח מקוצר)", brachaType: "al_hamichya", correctFoods: ["🥨", "🥐", "🍪", "🎂", "🍰", "🧇", "🍩", "🍕", "🍝", "🍇", "🫒"], wrongBrachaCategories: ["borei_nefashot_foods"], targetLength: 24, speed: 110, foodDropInterval: 2000, maxFoodItems: 7 },
            { name: "ברכה אחרונה: בּוֹרֵא נְפָשׁוֹת", brachaText: "בּוֹרֵא נְפָשׁוֹת רַבּוֹת וְחֶסְרוֹנָן... (נוסח מקוצר)", brachaType: "borei_nefashot", correctFoods: ["🍎","🍊","🍐","🍑","🍒","🍍","🥭","🥝","🥕","🍅","🥦","🌽","🥒","🥔","🌶️","🥬","🍓","🍉","🧅","🧄","🥛","🥚","🧀","🍄","🐟","🍬","🍫","🍿","🥤","🍭","🍯","🥩","🍗"], wrongBrachaCategories: ["al_hamichya_foods"], targetLength: 28, speed: 100, foodDropInterval: 1800, maxFoodItems: 8 }
        ];
        ...
        const ALL_FOOD_EMOJIS = {
            ...
            mezonot: ["🥨", "🥐", "🍪", "🎂", "🍰", "🧇", "🍩", "🍕", "🍝"],
            shehakol: ["🥛", "🥚", "🧀", "🍄", "🐟", "🍬", "🍫", "🍿", "🥤", "🍭", "🍯", "🥩", "🍗"],
            al_hamichya_foods: ["🥨", "🥐", "🍪", "🎂", "🍰", "🧇", "🍩", "🍕", "🍝", "🍇", "🫒"], // These are the "correct" ones for Al Hamichya level
            borei_nefashot_foods: ["🍎","🍊","🍐","🍑","🍒","🍍","🥭","🥝","🥕","🍅","🥦","🌽","🥒","🥔","🌶️","🥬","🍓","🍉","🧅","🧄","🥛","🥚","🧀","🍄","🐟","🍬","🍫","🍿","🥤","🍭","🍯","🥩","🍗"]
        };
        ...
        function handleFoodEaten(food, foodIndex) {
            ...
            if (food.brachaType === levelData.brachaType) { // Correct bracha
                ...
                if (score >= lastLifeMilestone + POINTS_FOR_EXTRA_LIFE) {
                    ...
                    updateLivesDisplay();
                    // Simple visual feedback for extra life (optional)
        ...
        function showTemporaryMessage(message, x, y, color = "black", duration = 1000) {
            ...
            tempCtx.font = 'bold ' + cellSize + 'px Arial';
        ...
        function loseLife() {
            ...
            if (lives <= 0) {
                ...
                // Reset snake position and clear some food as a penalty, or just pause briefly
        ...
        function spawnFoodItem() {
            ...
            let newFood;
            const levelData = LEVELS_DATA[currentLevelIndex];
            const isCorrectFood = Math.random() < 0.65; // 65% chance of correct food
            ...
            if (isCorrectFood) {
                ...
                brachaType = wrongCategoryKey.replace('_foods', ''); // e.g. 'al_hamichya_foods' -> 'al_hamichya'
            }
            ...
            while(!positionOK) {
                ...
                // Check against other food items
                if(positionOK) {
            ...
            foodItems.push({ x, y, emoji, brachaType });
        }
        ...
        touchControlsContainer.addEventListener('click', (e) => {
        ...
        function handleSwipe() {
            ...
            if (absDeltaX > absDeltaY) { // Horizontal swipe
                if (deltaX > 30 && direction !== 'left') direction = 'right'; // Swipe right
                else if (deltaX < -30 && direction !== 'right') direction = 'left'; // Swipe left
            } else { // Vertical swipe
                if (deltaY > 30 && direction !== 'up') direction = 'down'; // Swipe down
                else if (deltaY < -30 && direction !== 'down') direction = 'up'; // Swipe up
            }
        }
    ...
    if (url.pathname === "/" || url.pathname.startsWith("/index.html")) {
      return new Response(HTML_CONTENT, {
        headers: { 
          ...
          // Basic CSP - adjust if needed for more complex scenarios or external resources
          // For this self-contained game, it's mainly about inline scripts/styles and data URIs (if used for images, though SVGs are inline)
          // "Content-Security-Policy": "default-src 'self'; script-src 'self' 'unsafe-inline' https://cdnjs.cloudflare.com https://cdn.tailwindcss.com; style-src 'self' 'unsafe-inline' https://cdn.tailwindcss.com; font-src 'self' data:; img-src 'self' data:;"
        },
      });
    }
